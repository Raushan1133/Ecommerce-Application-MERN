      ...
      <div className='flex flex-col lg:flex-row gap-10 lg:justify-between p-4'>
        ...
        <div className='max-w-3xl w-full'>
            {
                loading ? (
                    loadingCart?.map((el,index)=>{
                        return(
                    <div key={"loadingitems"+index} className='w-full bg-slate-200 h-32 my-2 border border-slate-300 animate-pulse rounded'>
                    ...
                    return(
                        <div  key={product?._id+index} className='w-full bg-white h-32 my-2 border border-slate-300  rounded grid grid-cols-[128px,1fr]'>
                            ...
                            <div className='px-4 py-2 relative'>
                                ...
                                <p className='text-red-600 font-medium font-lg'>{dispalyINRcurrency(product?.productId?.sellingPrice)}</p>
                                <p className='text-slate-600 font-semibold font-lg'>{dispalyINRcurrency(product?.productId?.sellingPrice * product?.quantity)}</p>
                                </div>
                                 <div className='flex items-center gap-3 mt-1'>
                                    ...
                                    <button className='border border-red-600 text-red-600 w-6 h-6 text-center flex justify-center items-center rounded hover:bg-red-600 hover:text-white' onClick={()=>increaseQty(product?._id,product?.quantity)}>+</button>
                                 </div>
                            </div>
                        </div>
                    )
        ...
        <div className='h-36 bg-slate-200 border border-slate-300 animate-pulse'>
            ...
            <div className='h-fit bg-white'>
                ...
                <button className='bg-blue-600 p-2 text-white w-full bottom-0' onClick={handlePayment} >Make Payment</button>
            </div>
            )
        }
            </div>
